            ...
            <ul class="nav navbar-nav">
                ...
                <li class="dropdown user user-menu">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                        ...
                        <span class="hidden-xs"><?= isset($_SESSION['fullname']) ? $_SESSION['fullname'] : ''; ?>
                            <?php
                            include "../../config/koneksi.php";
                            $id = isset($_SESSION['id_user']) ? $_SESSION['id_user'] : null;

                            if ($id !== null) {
                                $query_verif = mysqli_query($koneksi, "SELECT * FROM petugas WHERE id = '$id'");
                                $row = mysqli_fetch_array($query_verif);

                                if ($row['id_user_lvl'] == 1) {
                                    echo "<i class='fa fa-check-circle' title='Administrator' data-toggle='tooltip' data-placement='bottom'></i>";
                                } elseif ($row['id_user_lvl'] == 2) {
                                    echo "<i class='fa fa-user-circle' title='Pengguna Biasa' data-toggle='tooltip' data-placement='bottom'></i>";
                                }
                            }
                            ?>
                        </span>
